...
// 	$Id: NumRecipes.cxx,v 1.1.1.1 2005/04/21 01:17:17 zhangy Exp $
//
// Description:
//
// Environment:
...
NumRecipes::gammp(double a, double x)
{
   ...
   if (x < 0.0 || a <= 0.0) std::cout<<"ErrMsg(error)" <<" Invalid arguments in routine gammp x=" << x << " a=" << a << endl;
   if (x < (a+1.0)) {
      gser(&gamser,a,x,&gln);
      return gamser;
   } else {
...
{
   std::cout<<"ErrMsg(fatal)" << " Numerical Recipes run-time error...\n" << c 
