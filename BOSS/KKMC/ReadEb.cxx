/****************************************
 * Read CMS energy from data base for 
 * psi(3770) production
 *
 * 2012-1-05 created pingrg 
 *
 ***************************************/

#include "GaudiKernel/Bootstrap.h"
#include "GaudiKernel/IJobOptionsSvc.h"
#include "GaudiKernel/ISvcLocator.h"
#include "GaudiKernel/PropertyMgr.h"


#include "KKMC/ReadEb.h"
#include <string>
#include <string.h>

int ReadEb::previousRun=-1;
double ReadEb::m_Ecms=3.773;
double ReadEb::m_xangle=0.011;


MYSQL* ReadEb::OpenDb() const {
  
  const char host[]     = "bes3db2.ihep.ac.cn";
  const char user[]     = "guest";
  const char passwd[]   = "guestpass";
  //const char db[]       = "run";
  const char db[]       = "offlinedb";
  unsigned int port_num = 3306;

  MYSQL* mysql = mysql_init(NULL);
  mysql = mysql_real_connect(mysql, host, user, passwd, db, port_num,
                             NULL,  // socket
                             0);    // client_flag

  if (mysql == NULL) {
    fprintf(stderr, "can not open database: offlinedb\n");
  }

  return mysql;
}


void ReadEb::CloseDb(MYSQL* mysql) const {
  mysql_close(mysql);
}


void ReadEb::ReadDb(int run){

  //read db use service
  Gaudi::svcLocator()->service("DatabaseSvc",m_dbsvc,true);
  //calibrated beam Energy
  if(m_usecbE){
    char stmt1[400];
    snprintf(stmt1, 1024,
	     "select beam_energy, px, py, pz "
	     "from RunParams664 where run_number = %d", run);//664 and 664p01 share the same database
    DatabaseRecordVector res;
    int row_no = m_dbsvc->query("offlinedb", stmt1, res);
    if(row_no==0){
      std::cout<<"Failed to read offline database"<<std::endl;abort();
    }
    
    DatabaseRecord* records = res[0];
    double bE=0;
    bE = records->GetDouble("beam_energy");
    m_beamE=bE;

    double px=records->GetDouble("px");
    double py=records->GetDouble("py");
    double pz=records->GetDouble("pz");

    m_beta.setX(px);
    m_beta.setY(py);
    m_beta.setZ(pz);
 
    m_Ecms = bE*2;
    m_xangle = px;
    // std::cout<<"beam e is:"<<bE<<", px="<<px<<",py="<<py<<",pz="<<pz<<std::endl;
  }
  //use online beam Energy 
  else{
    char stmt1[400];
    snprintf(stmt1, 1024,
	     "select BER_PRB, BPR_PRB "
	     "from RunParams where run_number = %d", run);
    DatabaseRecordVector res;
    int row_no = m_dbsvc->query("run", stmt1, res);
    if(row_no==0){
      std::cout<<"Failed to read online database"<<std::endl;abort();
    }
    
    DatabaseRecord* records = res[0];
    double E_E=0, E_P=0;
    E_E = records->GetDouble("BER_PRB");
    E_P = records->GetDouble("BPR_PRB");
    m_beamE=(E_E+E_P)/2.0;
    m_Ecms = m_beamE*2;
    m_xangle = 0.011;
  }

} 




