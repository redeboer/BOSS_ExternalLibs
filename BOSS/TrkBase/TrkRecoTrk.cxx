...
//      $Id: TrkRecoTrk.cxx,v 1.9 2008/09/23 00:58:03 zhangy Exp $
//
// Description:
...
// It's an error to try to store invalid fits
    std::cout<<"ErrMsg(error) "<< "Invalid fits cannot be marked for storage" << std::endl;
}
...
TrkRecoTrk::allReps() const
{
  ...
  //std::cout << " TrkRecoTrk::allReps" << std::endl;//yzhang debug
	    
  for (TrkRecoTrkImpl::repConstIter i=_impl->_reps.begin();i!=_impl->_reps.end();++i) {
          x->push_back(i->get());
	  /*
	  i->get()->printType(std::cout);//yzhang debug
	  i->get()->printAll(std::cout);//yzhang debug
	  */
  }
