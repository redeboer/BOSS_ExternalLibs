...
//--//Unify EeTo4e random engine with Bes random servive 
extern "C" {
  extern float flat_();
}
...
float flat_(){
  float rr;
  double rd=EeTo4eRandom::random();
//  std::cout<<"EeTo4eRandom::random()="<<rd<<endl;
  rr=(float)rd;
  return (float)EeTo4eRandom::random();
  //  return rr;
}
extern "C" {
  extern void intxs_();
...
EeTo4e::EeTo4e(const string& name, ISvcLocator* pSvcLocator):Algorithm( name, pSvcLocator )
{
  declareProperty("Ecms", m_Ecms = 3.65); // Ecm = sqrt(s) [GeV]
  declareProperty("cosee", m_cosee = 0.93); 
...
StatusCode EeTo4e::initialize(){
  ...
  log << MSG::INFO << "EeTo4e initialize" << endreq;
  ...
  CLHEP::HepRandomEngine* engine  = p_BesRndmGenSvc->GetEngine("EeTo4e");
  std::cout<<"==============================="<<engine<<endl;
  EeTo4eRandom::setRandomEngine(engine);
  //  *****************
...
StatusCode EeTo4e::finalize() 
{
  ...
  std::cout<< "EeTo4e finalized" << endl;
  return StatusCode::SUCCESS;
}
