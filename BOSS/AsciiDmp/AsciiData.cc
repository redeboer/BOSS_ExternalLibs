...
{
    x.check_start_tag(is,"MDCHIT");
    if(!x.initialized()) return is;
    is >> x.nHit;
    x.hitCol.resize(x.nHit);
    {for(int i = 0; i < x.nHit; i++) {
        is >> x.hitCol[i];
    }}
    x.check_end_tag(is, "MDCHIT");
    return is;
}

std::ostream& operator<<(std::ostream &os,const MDCHIT& x)
{
    os << std::endl << "{ MDCHIT" << std::endl;
    if(x.initialized()) {
    os << " " << x.nHit;
    {for(int i = 0; i < x.nHit; i++) {
        os << " " << x.hitCol[i];
...
std::istream& operator>>(std::istream& is, TOFHIT& x)
{
    ...
    x.check_end_tag(is, "TOFHIT");
    return is;
}

std::ostream& operator<<(std::ostream &os,const TOFHIT& x)
{
    os << std::endl << "{ TOFHIT" << std::endl;
    if(x.initialized()) {
    os << " " << x.nHit;
    {for(int i = 0; i < x.nHit; i++) {
        os << " " << x.hitCol[i];
...
std::istream& operator>>(std::istream& is, EMCHIT& x)
{
    ...
    x.check_end_tag(is, "EMCHIT");
    return is;
}

std::ostream& operator<<(std::ostream &os,const EMCHIT& x)
{
    os << std::endl << "{ EMCHIT" << std::endl;
    if(x.initialized()) {
    os << " " << x.nHit;
    {for(int i = 0; i < x.nHit; i++) {
        os << " " << x.hitCol[i];
...
std::istream& operator>>(std::istream& is, MUCHIT& x)
{
    ...
    x.check_end_tag(is, "MUCHIT");
    return is;
}

std::ostream& operator<<(std::ostream &os,const MUCHIT& x)
{
    os << std::endl << "{ MUCHIT" << std::endl;
    if(x.initialized()) {
    os << " " << x.nHit;
    {for(int i = 0; i < x.nHit; i++) {
        os << " " << x.hitCol[i];
...
std::istream& operator>>(std::istream& is, HitEVENT& x)
{
    ...
    try {
      ...
      std::cerr << "Got AsciiDumpException eror while reading mdc hit block !!!" << std::endl;
    }
    ...
    try {
      ...
      std::cerr << "Got AsciiDumpException eror while reading tof hiti block !!!" << std::endl;
    }
    ...
    try {
      ...
      std::cerr << "Got AsciiDumpException eror while reading emc hit block !!!" << std::endl;
    }
    ...
    try {
      ...
      std::cerr << "Got AsciiDumpException eror while reading muc hit block !!!" << std::endl;
    }
    x.check_end_tag(is, "HITEVENT");
    return is;
}

std::ostream& operator<<(std::ostream &os,const HitEVENT& x)
{
    os << std::endl << "{ HITEVENT" << std::endl;
    if(x.initialized()) {
    os << " " << x.header;
    os << " " << x.decayMode;
    os << " " << x.trackTruth;
