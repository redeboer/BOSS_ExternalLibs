//
// $Id: Helix.cxx,v 1.9 2012/05/29 09:03:55 mullrich Exp $
//
//   Class Helix 
//
...
//const double
//Helix::ConstantAlpha = 333.564095;
...
double *
Ext_Helix::x(double phi, double p[3]) const {
    //
    // Calculate position (x,y,z) along helix.
    //   
    // x = x0 + dr * cos(phi0) + (alpha / kappa) * (cos(phi0) - cos(phi0+phi))
...
HepPoint3D
Ext_Helix::x(double phi, HepSymMatrix & Ex) const {
    double x = m_pivot.x() + m_ac[0] * m_cp + m_r * (m_cp - cos(m_ac[1] +phi));
    double y = m_pivot.y() + m_ac[0] * m_sp + m_r * (m_sp - sin(m_ac[1] +phi));
    double z = m_pivot.z() + m_ac[3] - m_r * m_ac[4] * phi;
...
Hep3Vector
Ext_Helix::momentum(double phi) const {
    // 
    // Calculate momentum.
    //
    // Pt = | 1/kappa | (GeV/c)
...
Hep3Vector
Ext_Helix::momentum(double phi, HepSymMatrix & Em) const {
    // 
    // Calculate momentum.
    //
    // Pt = | 1/kappa | (GeV/c)
...
HepLorentzVector 
Ext_Helix::momentum(double phi, double mass) const {
    // 
    // Calculate momentum.
    //
    // Pt = | 1/kappa | (GeV/c)
...
HepLorentzVector 
Ext_Helix::momentum(double phi, double mass, HepSymMatrix & Em) const {
    // 
    // Calculate momentum.
    //
    // Pt = | 1/kappa | (GeV/c)
...
const HepPoint3D &
Ext_Helix::pivot(const HepPoint3D & newPivot) {
...
void
Ext_Helix::set(const HepPoint3D & pivot,
	   const HepVector & a,
	   const HepSymMatrix & Ea) {
    m_pivot = pivot;
    m_a = a;
...
Ext_Helix &
Ext_Helix::operator = (const Ext_Helix & i) {
...
void
Ext_Helix::updateCache(void) {
    //
    //   Calculate Ext_Helix center( xc, yc ).
    //   
...
HepMatrix
Ext_Helix::delApDelA(const HepVector & ap) const {
    //
...
HepMatrix
Ext_Helix::delXDelA(double phi) const {
    //
...
HepMatrix
Ext_Helix::delMDelA(double phi) const {
    //
...
HepMatrix
Ext_Helix::del4MDelA(double phi, double mass) const {
    //
...
HepMatrix
Ext_Helix::del4MXDelA(double phi, double mass) const {
    //
...
void
Ext_Helix::ignoreErrorMatrix(void) {
    m_matrixValid = false;
    m_Ea *= 0.;
}
