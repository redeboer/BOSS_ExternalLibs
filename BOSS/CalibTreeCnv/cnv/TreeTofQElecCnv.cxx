...
//static CnvFactory<TreeTofElecDataCnv> TofCal_factory;
//const  ICnvFactory& TreeTofElecDataCnvFactory = TofCal_factory;



TreeTofElecDataCnv::TreeTofElecDataCnv( ISvcLocator* svc) :
	TreeCalBaseCnv(svc, CLID_TofQ_Elec) { 
...
StatusCode TreeTofElecDataCnv::i_createObj(IOpaqueAddress* addr,
		DataObject*& refpObject) {

	MsgStream log(msgSvc(), "TreeTofElecDataCnv");
	...
	buf1->SetBuffer((*records)["BarBoardNum"],512000,kFALSE);
	buf2->SetBuffer((*records)["EndBoardNum"],512000,kFALSE);
	buf3->SetBuffer((*records)["QELecBarParEast"],512000,kFALSE);
	buf4->SetBuffer((*records)["QELecBarParWest"],512000,kFALSE);
	buf5->SetBuffer((*records)["QELecEndPar"],512000,kFALSE);
	buf6->SetBuffer((*records)["SimQELecBarParEast"],512000,kFALSE);
	buf7->SetBuffer((*records)["SimQELecBarParWest"],512000,kFALSE);
	buf8->SetBuffer((*records)["SimQELecEndPar"],512000,kFALSE);
	...
	TTree *BarBoardNum= new TTree();
	TTree *EndBoardNum= new TTree();
	TTree *QElecBarParEast = new TTree();
	TTree *QElecBarParWest = new TTree();
	TTree *QElecEndPar = new TTree();
	TTree *SimQElecBarParEast = new TTree();
	TTree *SimQElecBarParWest = new TTree();
	TTree *SimQElecEndPar = new TTree();
	BarBoardNum->Streamer(*buf1);
	EndBoardNum->Streamer(*buf2);
	QElecBarParEast->Streamer(*buf3);
	QElecBarParWest->Streamer(*buf4);
	QElecEndPar->Streamer(*buf5);
	SimQElecBarParEast->Streamer(*buf6);
	SimQElecBarParWest->Streamer(*buf7);
	SimQElecEndPar->Streamer(*buf8);
