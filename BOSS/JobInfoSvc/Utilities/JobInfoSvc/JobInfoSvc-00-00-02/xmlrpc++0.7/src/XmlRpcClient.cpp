...
//const char XmlRpcClient::METHODRESPONSE_TAG[] = "<methodResponse";//yzhang add
const char XmlRpcClient::METHODRESPONSE_TAG[] = "<methodResponse>";//yzhang delete
...
XmlRpcClient::execute(const char* method, XmlRpcValue const& params, XmlRpcValue& result)
{
  ...
  //std::cout<< "_connectionState   "<<_connectionState << std::endl;//yzhang debug
  ...
  if ( ! generateRequest(method, params))
    return false;
  result.clear();
  double msTime = 5.;   // Process until exit is called //yzhang change
  //double msTime = -1.0;   // Process until exit is called //yzhang delete
  _disp.work(msTime);

  if (_connectionState != IDLE )//yzhang add
  //if (_connectionState != IDLE || ! parseResponse(result))//yzhang delete
    return false; //yzhang delete
...
XmlRpcClient::parseResponse(XmlRpcValue& result)
{
  ...
  if ((XmlRpcUtil::nextTagIs(PARAMS_TAG,_response,&offset) &&
       ...
       // || XmlRpcUtil::nextTagIs(FAULT_TAG,_response,&offset) //yzhang delete
      && (_isFault = true))
  {
